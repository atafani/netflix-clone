    ...
    import { getCountries, getCountryCallingCode, formatPhoneNumberIntl} from 'react-phone-number-input'
    ...
    const Login: NextPage = () => {
        ...
        return (
            <div className="flex min-h-screen flex-row items-center justify-center bg-black sm:bg-[url('/images/login-bg.png')] bg-no-repeat bg-cover  relative md:py-20">
                ...
                <form action='#' className='w-full sm:w-[30rem] z-20 relative py-20 px-5 sm:px-16 pb-32'>
                    ...
                    <div className='relative z-40'>
                            ...
                            <Controller control={control} name="emailPhone"
                                ...
                                render={({
                                    ...
                                    <div className='relative my-4 '>
                                        <div className={` w-full text-xs text-gray-100 sm:text-lg rounded-md border border-[#333333] appearance-none  bg-[#333333] ${error ? " border-b-orange-400" : ""} flex flex-row items-center`}>
                                            <input
                                                type='text'
                                                id='emailPhone'
                                                value={value}
                                            ...
                                            />
                                            {/^[0-9]+$/.test(value) && <p className="text-sm text-gray-400  align-middle hover:cursor-pointer" onClick={() => setIsCountrySelectShown(!isCountrySelectShown)}>{`+${getCountryCallingCode(country)}`}<FaCaretDown className="inline"/></p>}
                                            <div className={`absolute top-full left-0 right-0 h-[15rem] z-30 bg-neutral-600 overflow-y-scroll p-5 rounded-md transition-all duration-400 slide-down  ${isCountrySelectShown ? 'h-100' : 'h-0 p-0'}`}>
                                                {getCountries().map((c: any) => <p key={c} className="py-2 px-3 hover:bg-neutral-500 hover:cursor-pointer text-sm" onClick={() => {setCountry(c);setIsCountrySelectShown(false)}}>{regionNames.of(c)}</p>)}
                                            </div>
                                            <label
                                                htmlFor='email'
                                        ...
                                        {error && <p className="text-orange-500 text-xs mt-2">{error.message}</p>}
                                    </div>
                                )} />
                            <Controller control={control}
                                name="password"
                                rules={{ required: { value: true, message: 'Please enter a valid password.' }, minLength: { value: 4, message: 'Your password must contain between 4 and 60 characters.' },maxLength: { value: 60, message: 'Your password must contain between 4 and 60 characters.' } }}
                                render={({
                                    field: { onChange, onBlur, value, name, ref },
                                    fieldState: { invalid, isTouched, isDirty, error },
                                    formState,
                                    ...
                                    <input
                                        ...
                                        onChange={onChange}
                                        className={`block px-5 pb-1 pt-5 w-full text-xs text-gray-100 sm:text-lg rounded-md border border-[#333333]  appearance-none focus:outline-none focus:ring-0 peer bg-[#333333] ${error ? " border-b-orange-400" : ""}`}
                        ...
                        <p className="text-gray-500 my-5 text-md">New to Netflix? <Link href='register'><span className="text-white">Sign up now</span></Link>.</p>
                        <p className="text-gray-500 my-3 text-sm">This page is protected by Google reCAPTCHA to ensure you're not a bot. <Link href='#'><span className="text-blue-500">Learn more.</span></Link></p>
                    </div>
                </form>
            </div>
        );
